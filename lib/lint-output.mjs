...
function isCodeFile(name) {
  return /\.(html|js|mjs|css)$/.test(name);
}
...
export function sdkBindings() {
  return {
    methods: [...METHOD_ALLOWLIST],
    events: [...EVENT_ALLOWLIST],
    screens: ['MAIN', 'CHAT', 'NEW_CHAT', 'CREATE_GROUP', 'CALL', 'RING', 'INCOMING', 'DIALER', 'PAIRING', 'IDENTITIES'],
    gates: {},
  };
}

/**
 * Lint skin files against the SDK allowlist.
 * @returns {{ ok: boolean, errors: { file: string, message: string }[], warnings: { file: string, message: string }[] }}
 */
export function lintOutput(files, compiled = sdkBindings()) {
  const errors = [];
  ...
  for (const [file, raw] of Object.entries(files)) {
    ...
    if (/\bnew\s+WebSocket\b/.test(source)) {
      fail(file, 'WebSocket belongs on window.top.controller (startBrowserPairing), not in skin HTML.');
    }
    ...
    for (const name of members) {
      ...
      if (!allowedMethods.has(name)) {
        fail(file, `controller.${name} is not on the SDK allowlist`);
      }
    }
    ...
    while ((match = CONTROLLER_ON.exec(source))) {
      ...
      if (!EVENT_ALLOWLIST.includes(eventName)) {
        ...
        fail(file, `event "${eventName}" is not on the SDK allowlist`);
      }
    }
