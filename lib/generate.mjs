...
.topbar, .composer, .panel { padding: 12px 16px; }
.topbar { display: flex; align-items: center; gap: 10px; border-bottom: 1px solid var(--line); min-height: 56px; }
.topbar h1 { font-size: 17px; margin: 0; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.icon-btn, .btn { border: 0; background: var(--fill); color: var(--text); border-radius: 10px; padding: 8px 12px; font-size: 14px; cursor: pointer; }
.btn.primary { background: var(--accent); color: #fff; }
.btn.danger { background: var(--danger); color: #fff; }
.btn.ok { background: var(--ok); color: #fff; }
.list { list-style: none; margin: 0; padding: 0; }
.row { padding: 14px 16px; border-bottom: 1px solid var(--line); cursor: pointer; display: flex; justify-content: space-between; gap: 12px; }
...
.bubble { max-width: 80%; padding: 8px 12px; border-radius: 16px; background: var(--fill); align-self: flex-start; }
.bubble.mine { background: var(--accent); color: #fff; align-self: flex-end; }
.composer { display: flex; gap: 8px; border-top: 1px solid var(--line); }
.composer input, .field { flex: 1; border: 1px solid var(--line); background: var(--bg); color: var(--text); border-radius: 12px; padding: 10px 12px; font-size: 16px; }
.column { display: flex; flex-direction: column; height: 100%; }
.banner { margin: 24px; color: var(--muted); }
.call-stage { min-height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: space-between; padding: 48px 20px 32px; background: linear-gradient(160deg, #7342a0, #3634a3); color: #fff; }
.call-stage h2 { font-weight: 600; font-size: 28px; text-align: center; }
...
select.field { width: 100%; margin-bottom: 10px; }
`;
}
...
function appHtml(compiled) {
  ...
  const video = compiled.spec.calls.video;
...
function buildQuery(screen, extra) {
  ...
  for (const key of ['sessionId', 'remoteId', 'sessionName', 'from', 'to', 'videoCall']) {
...
requireController();
loadScreen(pageParams().screen || 'MAIN', pageParams());
...
function mainscreenHtml(compiled) {
  const name = compiled.spec.app?.name || 'Chats';
...
<div class="column">
  ...
  <p class="empty" id="empty">No conversations yet.</p>
</div>
`, `
requireController();
...
async function render() {
  ...
  sessions.forEach(function (session) {
    ...
    li.innerHTML = '<div><div>' + title + '</div><div class="meta">' + (session.lastMessage || '') + '</div></div>';
...
async function loadHistory() {
  ...
  messages.forEach(function (msg) {
    appendBubble(msg.body || msg.text || '', !!msg.outgoing);
  });
...
document.getElementById('sendBtn').addEventListener('click', async function () {
  ...
  if (sessionId) controller.sendMessage(sessionId, text, true);
  else if (params.remoteId) controller.sendMessage(params.remoteId, text, false);
});
controller.on('new-message', function (data) {
  const body = (data && data.body) ? data.body : (data || {});
  if (sessionId && String(body.sessionId) !== String(sessionId)) return;
  appendBubble(body.body || body.text || '', false);
});
...
controller.on('chatstate', function (data) {
  ...
  if (body.state === 'composing') document.getElementById('title').dataset.typing = '1';
});
` : ''}
...
document.getElementById('attachBtn').addEventListener('click', function () {
  ${has(compiled, 'imagePicker') ? 'controller.imagePicker();' : 'controller.camera();'}
});
` : ''}
${compiled.events.includes('image-picker-result') && has(compiled, 'sendFileMessage') ? `
controller.on('image-picker-result', function (data) {
  const body = (data && data.body) ? data.body : {};
  if (sessionId && body.fileId) controller.sendFileMessage(sessionId, body.fileId, body.caption || '');
});
` : ''}
${has(compiled, 'downloadFile') ? `function saveFile(id) { controller.downloadFile(id); }` : ''}
...
document.getElementById('callBtn').addEventListener('click', function () {
  ...
  navigate('RING', { to: params.remoteId, sessionId: sessionId, sessionName: params.sessionName, videoCall: false });
});
` : ''}
${video ? `
document.getElementById('videoBtn').addEventListener('click', function () {
  ...
  navigate('RING', { to: params.remoteId, sessionId: sessionId, sessionName: params.sessionName, videoCall: true });
});
` : ''}
${gate && (voice || video) ? `
if (isWhatsApp()) {
...
${has(compiled, 'endCall') ? `document.getElementById('endBtn').addEventListener('click', function () { controller.endCall(); navigate('MAIN'); });` : ''}
${has(compiled, 'setMute') ? `document.getElementById('muteBtn').addEventListener('click', function () { controller.setMute(true); });
controller.on('mute-state', function () {});` : ''}
${has(compiled, 'setHold') ? `document.getElementById('holdBtn').addEventListener('click', function () { controller.setHold(true); });
controller.on('local-hold-state', function () {});
controller.on('active-calls', function () {});` : ''}
${video && has(compiled, 'switchCamera') ? `document.getElementById('camBtn').addEventListener('click', function () { controller.switchCamera(); });` : ''}
...
document.getElementById('audioBtn').addEventListener('click', async function () {
  ...
  if (devices && devices[0]) controller.changeAudioDevice(devices[0]);
});
` : ''}
${has(compiled, 'sendDtmf') ? `document.getElementById('dtmfBtn').addEventListener('click', function () { controller.sendDtmf('#'); });` : ''}
${has(compiled, 'transferCall') ? `document.getElementById('xferBtn').addEventListener('click', function () { controller.transferCall(params.remoteId); });` : ''}
...
document.getElementById('endBtn').addEventListener('click', function () {
  ${has(compiled, 'endCall') ? 'controller.endCall();' : ''}
  navigate('MAIN');
});
...
document.getElementById('acceptBtn').addEventListener('click', function () {
  ${has(compiled, 'acceptCall') ? 'controller.acceptCall();' : ''}
  navigate('CALL', { to: params.from, sessionId: params.sessionId, sessionName: params.sessionName, videoCall: params.videoCall });
});
document.getElementById('rejectBtn').addEventListener('click', function () {
  ${has(compiled, 'rejectCall') ? 'controller.rejectCall();' : ''}
  navigate('MAIN');
});
${compiled.events.includes('call-ended') ? `controller.on('call-ended', function () { navigate('MAIN'); });` : ''}
...
<div class="column">
  ...
  <p class="banner">Browser-relay needs the Arnacon phone app online on the same room. Pairing is owned by the native shell — this screen does not open its own signaling socket.</p>
</div>
`, `
document.getElementById('backBtn').addEventListener('click', function () { navigate('MAIN'); });
`);
...
export function generateFiles(compiled) {
  ...
  if (hasScreen(compiled, 'CALL')) {
    ...
    if (compiled.spec.calls.video) files['videocall.html'] = callHtml(compiled, { video: true });
  }
